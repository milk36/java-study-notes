...
* Memory Fragmentation 内存碎片化 
  ...
  因为 CMS使用的是MarkSweep ,所以很容易产生碎片
* Floating Garbage 浮动垃圾

  1. Concurrent Mode Failure:

      如果并发收集器无法在年老代填满之前完成不可达对象的回收，或者如果年老代中可用的空闲空间块不能满足分配，那么应用程序将暂停，并在停止所有应用线程的情况下完成收集
  ...
  1. PromotionFailed
      ...
      `–XX:CMSInitiatingOccupancyFraction` 默认:92% 产生FGC; 可以降低这个值，让CMS保持老年代足够的空间 留给浮动垃圾
* CMS 在特殊情况下会启用Serial Old来执行垃圾回收操作 ; 这时候的GC性能就非常底
